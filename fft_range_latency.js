...
const maxNotes = 1000; // maximum number of notes to add for each interval
const minIntensity = 110; // minimum volume for note to be added (0-255)
...
el.on('change', function(e){
  ...
  reader.onload = function (e) {
    ...
    context.decodeAudioData(e.target.result, function(buffer){
      ...
      // audioSourceL.connect(context.destination);
      // audioSourceR.connect(context.destination);
      async function process() {
        ...
        while (added < maxNotes) {
          ...
          if (note != undefined && fL[i][1] > minIntensity) {
            song.addNote(new Note(song, note, time, 1 / resolution + (1 / resolution * 0.1), shouldUseHigher ? 15 : 13, (fL[i][1] - minIntensity) / (255 - minIntensity)));
            added++;
            s += "[" + note + " " + fL[i][1] + "] ";
          }
          i++;
        ...
        while (added < maxNotes) {
          ...
          if (note != undefined && fR[i][1] > minIntensity) {
            song.addNote(new Note(song, note, time, 1 / resolution + (1 / resolution * 0.1), shouldUseHigher ? 14 : 16, (fR[i][1] - minIntensity) / (255 - minIntensity)));
            added++;
            s += "[" + note + " " + fR[i][1] + "] ";
          }
          i++;
        ...
        window.setTimeout(process, song.sleepTime / resolution / 2);
      }
      process();
